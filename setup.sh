...
then
	...
	su - root -c "cat ${PWD}/admin-packages.txt | xargs pkg install"
...
then
	...
	su - root -c "pkg update -f"
	ee $? "Unable to update package cache"
	su - root -c "cat ${PWD}/desktop-packages.txt | xargs pkg install"
	ee $? "Unable to install admin packages"
	echo "Enabling sound"
...
then
	...
	su - root -c "pkg update -f"
	ee $? "Unable to update package cache"
	su - root -c "cat ${PWD}/bash-packages.txt | xargs pkg install"
...
then
	...
	su - root -c "pkg update -f"
	ee $? "Unable to update package cache"
	su - root -c "cat ${PWD}/intel-xorg-packages.txt | xargs pkg install"
